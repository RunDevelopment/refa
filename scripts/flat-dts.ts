...
function ensureUniqueNames(parsed: ParsedDts): void {
	...
	for (const modName in parsed.modules) {
		...
		for (const mem of mod.members) {
			const collision = names.get(mem.name);
			if (collision && collision.modName !== modName) {
				throw new Error(
					`The name ${JSON.stringify(mem.name)} is present in both ${JSON.stringify(
						modName
...
function ensureSoleExport(parsed: ParsedDts): void {
	...
	for (const modName in parsed.modules) {
		...
		for (const { from } of mod.exports) {
			...
			if (alsoExports !== undefined) {
				throw new Error(
					`The both "${modName}" and "${alsoExports}" export "${from}". At most one module is allowed to export another module.`
				);
			}
...
function parseModuleContent(modSource: SourceCode): ParsedModule {
	...
	const moduleComment = modSource.peek(JS_M_COMMENT)?.[0];
	if (moduleComment !== undefined && /^\s*(?:\*\s*)?@module\b/m.test(moduleComment)) {
	...
	while (modSource.good) {
		...
		modSource.consumeAnyOf(
			[
				{
					pattern: reBuild(/export\s*\*\s*(?:as\s+(<<JS_IDENT>>)\s*)?from\s*(<<JS_STRING>>);?/y, {
						JS_STRING,
						JS_IDENT,
					}),
...
function parseMember(memSource: SourceCode): Member {
	const [, type, name] = memSource.consume(
		reBuild(/(?=(function|interface|class|type|var|let|const|namespace)\s+(<<JS_IDENT>>))/y, { JS_IDENT }),
		"Invalid member"
	);
...
function consumeTSUntil(source: SourceCode, stop: string): SourceCode {
	...
	return source.consumeAnyUntil(
		[
			...
			{
				pattern: reBuild(/(<<disallowed>>|<<JS_STRING>>|<<JS_COMMENT>>)+/my, {
					JS_STRING,
					JS_COMMENT,
					disallowed,
				}),
...
class SourcePositionTranslator {
	...
	at(pos: number): { line: number; column: number } | undefined {
		if (pos < 0 || pos > this._length) {
			...
			const lineIndex = findLastIndex(this._lineOffsets, offset => pos >= offset);
			const column = pos - this._lineOffsets[lineIndex];
...
function reBuild(base: RegExp, subs: Record<string, RegExp>): RegExp {
	const source = base.source.replace(/<<(\w+)>>/g, (_, name) => {
		const sub = subs[name] as RegExp | undefined;
		if (!sub) {
