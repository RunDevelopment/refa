...
module.exports["Transformers >> union-characters >> Prism regex snapshot"] = `
/<!--[^]*?-->/i
/<\\?[^]+?\\?>/i
/<!\\[CDATA\\[[^]*?\\]\\]>/i
/<!DOCTYPE(?:[^"'>[\\]]|"[^"]*"|'[^']*')+(?:\\[(?:[^"'<\\]]|"[^"]*"|'[^']*'|<(?!!--)|<!--(?:[^-]|-(?!->))*-->)*\\]\\s*)?>/i
...
/\\bcl_(?:GLenum|GLint|GLuin|addressing_mode|bitfield|bool|buffer_create_type|build_status|channel_(?:order|type)|(?:u?(?:char|short|int|long)|float|double)(?:[2348]|16)?|command_(?:queue(?:_info|_properties)?|type)|context(?:_info|_properties)?|device_(?:exec_capabilities|fp_config|id|info|local_mem_type|mem_cache_type|type)|(?:event|sampler)(?:_info)?|filter_mode|half|image_info|kernel(?:_info|_work_group_info)?|map_flags|mem(?:_flags|_info|_object_type)?|platform_(?:id|info)|profiling_info|program(?:_build_info|_info)?)\\b/
/\\bCL_(?:TRUE|FALSE)\\b/
...
/\\b(?:AT|BOOL|BYTE|[DL]?WORD|U?[DLS]?INT|L?REAL|TIME(?:_OF_DAY)?|TOD|DT|DATE(?:_AND_TIME)?|STRING|ARRAY|ANY|POINTER)\\b/
/%[IMQ][BDLWX][\\d.]*|%[IQ][\\d.]*/
/\\b(?:16#[\\dA-F]+|2#[01_]+|0X[\\dA-F]+)\\b|\\b(?:[DT]|DT|TOD)#[\\d:DHMS_]*|\\b[A-Z]*#[\\d,._]*|(?:\\b\\d+\\.?\\d*|\\B\\.\\d+)(?:E[-+]?\\d+)?/i
/\\b(?:TRUE|FALSE|NULL)\\b/
