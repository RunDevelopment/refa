...
module.exports[n`Transformers >> remove-unnecessary-assertions >> /\w+\b(?=\s*%)/ `] = lit`
/\w+(?=\s*%)/
`;

module.exports[n`Transformers >> remove-unnecessary-assertions >> /\w+(?=\s*%)\b/ `] = lit`
/\w+(?=\s*%)/
`;
