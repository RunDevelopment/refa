...
export function nestedQuantifiers(options?: Readonly<CreationOptions>): PureTransformer {
	...
	return {
		onQuantifier({ node }: NodeObject<Quantifier>, { signalMutation }: TransformContext): void {
			if (node.max === 0) {
				return;
			}
